...
sealed trait ScoredStream[A] {
  ...
  // TODO scoping
  protected[ordered] def mapScoredAux[B](f: A => Scored[B], candidateHead: Scored[B]): ScoredCons[B]

  def flatMap[B](f: A => ScoredStream[B]): ScoredStream[B]
  protected[ordered] def flatMapAux[B](f: A => ScoredStream[B], candidateHeadStream: ScoredStream[B]): ScoredStream[B]
  ...
  // protected[ordered] def append(other: => ScoredStream[A]): ScoredStream[A]
}
...
object ScoredStream extends ScoredStreamInstances {
  ...
  protected[ordered] def fromSortedSeq[A](s: Seq[Scored[A]]): ScoredStream[A] = {
...
class ScoredNil[A]() extends ScoredStream[A] {
  ...
  // override protected[ordered] def append(other: => ScoredStream[A]): ScoredStream[A] =
  //   other
}
...
class ScoredCons[A] protected[ordered] (
  val head: Scored[A],
